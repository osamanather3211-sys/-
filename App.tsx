...
const App: React.FC = () => {
  ...
  // Leak Detection & Alert Logic - Decoupled to avoid render blocking
  useEffect(() => {
    ...
    if (currentFlowRate > 0 && currentFlowRate < 10) {
      if (!leakTimerRef.current) {
        leakTimerRef.current = setTimeout(() => {
            // Check current value from ref or state in functional update if possible, 
            // but for simplicity we assume state is stable enough for this check
            if (isLeaking) { // Double check consistency
      ...
      // Clear leak alert if flow stops
      if (!isLeaking) {
         setAlerts(prev => {
  ...
  }, [simState.currentFlowRate, simState.isLeaking, Math.floor(simState.totalToday), dailyGoal]); // Rounded totalToday to reduce effect firing
        ...
        case AppView.SETTINGS:
            return userSettings ? (
                <Settings 
                  ...
                  onUpdateSettings={(s) => { setUserSettings(s); playSfx('success'); }}
                  dailyGoal={dailyGoal}
  ...
  if (!userSettings) {
    return <Onboarding onComplete={(s) => { playSfx('success'); setUserSettings(s); }} />;
  }
